...
const filterOrderType = () => {
    if (sessionStorage.getItem("curCheckState")) {
        switch (sessionStorage.getItem("curCheckState")) {
            case "bzj-checkPending": return "待审核";
            case "bzj-unapprove": return "审核未通过";
            case "bzj-checkFail": return "审核失败";
            case "bzj-checkSuccess": return "审核通过";
            default: return "";
        }
    }
}

require("./cashDepositDetails.less")
...
class CheckPendingDetails extends React.Component {
    ...
    refresh = () => {
        ...
        //获取当前页配资详情
        getFundingOrderDetails(JSON.parse(sessionStorage.getItem("curCheckPending")).id, "/bzj/query", (action) => {
            ...
            if (this.props.fundingOrderDetails.code == 0) {
                ...
                console.log("保证金详情", this.props.fundingOrderDetails);
            }
                    ...
                    <PageHeader
                        ...
                        content={
                            <Collapse onChange={() => { this.setState({ nextBtnDisable: false }) }}>
                                <Panel header={
                                    <h3 style={{ marginBottom: 0, fontWeight: "bold" }}>保证金审核详情页</h3>
                                }>
                                    <Collapse accordion>
                                        {
                                                                    ...
                                                                    <Step key={index} style={{ minHeight:120 }} title={item.title} icon={item.auditState == "FAIL" ? <Icon type="warning" style={{ color: "orange", fontSize: 32 }} /> : ""} status={this.filterStepState(item.auditState)}
                                                                        description={
                                                                            <div>
                                                                                ...
                                                                                {item.canAudit ? item.auditState == "AUDIT" ? "" : <Button disabled={nextBtnDisable} type="primary" onClick={() => { this.showModal(item.hint, item.step, value.instanceId, "审核界面") }}>确认</Button> : ""}
                                                                            </div>
                                                                        }
                                                                    />
                                                                )
