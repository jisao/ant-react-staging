...
class checkPending extends React.Component {
    state = {
        ...
        visible: false,
        itemList: [],
        url: "/allocation/auditList",//请求的接口
        ...
        columns: [{
            ...
            dataIndex: 'period',
            key: 'period',
            render: (item, record) =>this.filterDate(item),
        }, {
            title: '审核进度',
            dataIndex: 'stepText',
            key: 'stepText'
    ...
    onTabChange = (key) => {
        ...
        switch (key) {
            ...
            case "待审核详情":
                this.state.url = "/allocation/auditList";
                this.curCheckState = "checkPending";
                break;
            case "审核未通过详情":
                this.state.url = "/allocation/rejectList";
                this.curCheckState = "unapprove";
                break;
            case "审核失败详情":
                this.state.url = "/allocation/failList";
                this.curCheckState = "checkFail";
                break;
            case "审核成功详情":
                this.state.url = "/allocation/sucList";
                this.curCheckState = "checkSuccess";
                break;
        }
        this.setState({
            url: this.state.url,
            curCheckState: this.state.curCheckState
        })
    ...
    linkDetails = (record) => {
        console.log(record);
        console.log('当前需要审核的单子的详情',this.state.curCheckState);
        ...
        this.props.history.push("/main/checkPendingDetails")
    }
