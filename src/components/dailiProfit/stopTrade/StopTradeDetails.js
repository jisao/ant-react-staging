...
require("./StopTradeDetails.less")

//返回当前详情页面的type
const filterOrderType = () => {
    if (sessionStorage.getItem("curCheckState")) {
        switch (sessionStorage.getItem("curCheckState")) {
            case "stoptrade-checkPending": return "待审核";
            case "stoptrade-outPending": return "出金审核";
            case "stoptrade-unapprove": return "审核未通过";
            case "stoptrade-checkFail": return "审核失败";
            case "stoptrade-checkSuccess": return "审核通过";
            default: return "";
        }
    }
}
...
class CheckPendingDetails extends React.Component {
    ...
    refresh = () => {
        ...
        //获取当前页配资详情
        getFundingOrderDetails(JSON.parse(sessionStorage.getItem("curCheckPending")).id, "/stoptrade/query", (action) => {
            ...
            if (this.props.fundingOrderDetails.code == 0) {
                ...
                console.log("终止操盘详情页面", this.props.fundingOrderDetails);
            }
                    ...
                    <PageHeader
                        ...
                        content={
                            <Collapse onChange={() => { this.setState({ nextBtnDisable: false }) }}>
                                <Panel header={
                                    <h3 style={{ marginBottom: 0, fontWeight: "bold" }}>终止交易审核详情页</h3>
                                }>
                                    <Collapse accordion>
                                        {
                                                                    ...
                                                                    <Step key={index} style={{ minHeight:120 }} title={item.title} icon={item.auditState == "FAIL" ? <Icon type="warning" style={{ color: "orange", fontSize: 32 }} /> : ""} status={this.filterStepState(item.auditState)}
                                                                        description={
                                                                            <div>
                                                                                ...
                                                                                {item.canAudit ? item.auditState == "AUDIT" ? "" : <Button type="primary" disabled={nextBtnDisable} onClick={() => { this.showModal(item.hint, item.step, value.instanceId, "审核界面") }}>确认</Button> : ""}
                                                                            </div>
                                                                        }
                                                                    />
                                                                )
