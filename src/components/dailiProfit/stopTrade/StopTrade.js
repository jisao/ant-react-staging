...
class StopTrade extends React.Component {
    state = {
        data: [],
        pagination: {
            current: 1,
        ...
        visible: false,
        itemList: [],
        url: "/stoptrade/auditList",//请求的接口
        curTableType: "待审核详情",//当前table的类型
        curCheckState: "checkPending",//跳转到详情页面需要存的详情类型
        columns: [{
            title: '交易ID',
            dataIndex: 'accountId',
            key: 'accountId',
    ...
    onTabChange = (key) => {
        ...
        switch (key) {
            // /allocation/auditList
            // /allocation/failList
            // /allocation/sucList
            // /allocation/rejectList
            case "待审核详情":
                this.state.url = "/stoptrade/auditList";
                this.curCheckState = "stoptrade-checkPending";
                break;
            case "出金审核详情":
                this.state.url = "/stoptrade/cashList";
                this.curCheckState = "stoptrade-outPending";
                break;
            case "审核未通过详情":
                this.state.url = "/stoptrade/rejectList";
                this.curCheckState = "stoptrade-unapprove";
                break;
            case "审核失败详情":
                this.state.url = "/stoptrade/failList";
                this.curCheckState = "stoptrade-checkFail";
                break;
            case "审核成功详情":
                this.state.url = "/stoptrade/sucList";
                this.curCheckState = "stoptrade-checkSuccess";
                break;
        }
        this.setState({
            url: this.state.url,
    ...
    linkDetails = (record) => {
        ...
        this.props.history.push("/main/stopTradeDetails")
    }
...
export default connect(filterCheckPendingList)(StopTrade)