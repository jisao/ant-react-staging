...
require("./extractGainDetails.less")

//返回当前详情页面的type
const filterOrderType = () => {
    if (sessionStorage.getItem("curCheckState")) {
        switch (sessionStorage.getItem("curCheckState")) {
            case "withdraw-checkPending": return "待审核";
            case "withdraw-outPending": return "出金审核";
            case "withdraw-unapprove": return "审核未通过";
            case "withdraw-checkFail": return "审核失败";
            case "withdraw-checkSuccess": return "审核通过";
            default: return "";
        }
    }
}
...
class CheckPendingDetails extends React.Component {
    state = {
        ...
        uploadImgs: [],//把上传图片的地址保存起来
        radioValue: "审核通过",//默认的单选框
        fileList: [],//上传文件所存在的数组
    ...
    refresh = () => {
        ...
        //获取当前页配资详情
        getFundingOrderDetails(JSON.parse(sessionStorage.getItem("curCheckPending")).id, "/withdraw/query", (action) => {
            ...
            if (this.props.fundingOrderDetails.code == 0) {
                ...
                console.log("提盈审核详情", this.props.fundingOrderDetails);
            }
        ...
        getCheckPendingDetails(obj, (action) => {
            ...
            console.log(JSON.parse(sessionStorage.getItem("curCheckPending")));
            console.log("待审核流程数据", this.props.cpDetails.data);
            console.log("待审核详情数据", this.state.itemList);
        })
    }
    componentWillMount() {
    ...
    render() {
        ...
        let { detailsResult, itemList, breadcrumbList, details, currentId, visible, loadingModal, previewVisible, previewImage, fileList, curStepMsg, unfoldKey, visibleRemark, modalTitle, nextBtnDisable } = this.state
            ...
            <div className="checkPengdingDetails_container">
                <Spin size="large" spinning={this.state.loading}>
                    ...
                    <PageHeader
                        ...
                        content={
                            <Collapse onChange={() => { this.setState({ nextBtnDisable: false }) }}>
                                <Panel header={
                                    <h3 style={{ marginBottom: 0, fontWeight: "bold" }}>提盈审核详情页</h3>
                                }>
                                    <Collapse accordion>
                                        {
                                                                    ...
                                                                    <Step key={index} style={{ minHeight:120 }} title={item.title} icon={item.auditState == "FAIL" ? <Icon type="warning" style={{ color: "orange", fontSize: 32 }} /> : ""} status={this.filterStepState(item.auditState)}
                                                                        description={
                                                                            <div>
                                                                                ...
                                                                                {item.canAudit ? item.auditState == "AUDIT" ? "" : <Button disabled={nextBtnDisable} type="primary" disabled={nextBtnDisable} onClick={() => { this.showModal(item.hint, item.step, value.instanceId, "审核界面") }}>确认</Button> : ""}
                                                                            </div>
                                                                        }
                                                                    />
                                                                )
                    ...
                    >
                        ...
                        {
                            modalTitle == "审核界面" ? <RadioGroup name="radiogroup" onChange={this.selectRadio} style={{ lineHeight: "30px" }} value={this.state.radioValue}>
