...
const filterOrderType = () => {
    if (sessionStorage.getItem("curCheckState")) {
        switch (sessionStorage.getItem("curCheckState")) {
            case "managefee-checkPending": return "待审核";
            case "managefee-unapprove": return "审核未通过";
            case "managefee-checkFail": return "审核失败";
            case "managefee-checkSuccess": return "审核通过";
            default: return "";
        }
    }
}

require("./checkManageFee.less")
                                                                    ...
                                                                    <Step key={index} style={{ minHeight: 120 }} title={item.title} icon={item.auditState == "FAIL" ? <Icon type="warning" style={{ color: "orange", fontSize: 32 }} /> : ""} status={this.filterStepState(item.auditState)}
                                                                        description={
                                                                            <div>
                                                                                ...
                                                                                {item.canAudit ? item.auditState == "AUDIT" ? "" : <Button  type="primary" onClick={() => { this.showModal(item.hint, item.step, value.instanceId, "审核界面") }}>确认</Button> : ""}
                                                                            </div>
                                                                        }
                                                                    />
                                                                )
