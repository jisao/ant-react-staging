...
import { getIndexMsg, addIndexMsg, changeIndexMsg, removeIndexMsg } from "../../redux/actions/acApp/acIndexMsg";
...
class IndexMsg extends React.Component {
                ...
                <div id="bodyDivSlide" className="tableBox" >
                    <Tabs defaultActiveKey="1" type="card" activeKey={this.state.activeKey} onChange={this.tabsOnChange}>
                        ...
                        <TabPane tab={this.state.tab2Name} key="2" className="inputDiv">
                            ...
                            <p><label>排序位置：</label><Input ref='sort' size="large" type="text" placeholder="" /></p>
    ...
    refresh = () => {
        ...
        getIndexMsg((action) => {
            ...
            // console.log(this.props.indexMsg)
            this.setState({
                loading: false,
                itemList: this.props.indexMsg.data.result
            })
    ...
    remove = (record) => {
        removeIndexMsg(record.id, (action) => {
            this.props.dispatch(action)
            this.refresh()
        })
    }
    ...
    handleOk = () => {
        ...
        if (this.state.tab2Name == '新增配置') {
            ...
            addIndexMsg(item, (action) => {
                this.props.dispatch(action)
                if (this.props.indexMsg.code == 0) {
                    this.handleCancel()
                    this.refresh()
                } else {
                    this.openNotificationWithIcon("error", this.props.indexMsg.message)
                }
            })
...
function filterlist(store) {
    // console.log(store);
    return {
        indexMsg: store.indexMsg
    }
}

export default connect(filterlist)(IndexMsg)
